...
<!-- Toolbar tint probes. iOS 26 dropped <meta name="theme-color"> (it still
     ...
     These two strips give Safari something honest to sample. The script below
     paints each one with whatever colour is ACTUALLY rendered at that edge, so
     they are invisible by construction. They are iOS-only (see style.css) and
     sit under the translucent chrome thanks to viewport-fit=cover. -->
...
<script>
  ...
  const tintTop = document.querySelector(".tint-top");
  ...
  const paint = () => {
    queued = false;
    tintTop.style.backgroundColor = paintedAt(2);
